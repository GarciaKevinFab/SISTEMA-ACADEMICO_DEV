...
const enrichUser = (u) => {
  ...
  const finalPerms = new Set([...serverPerms, ...rolePerms]);
...
export const AuthProvider = ({ children }) => {
  ...
  // ✅ NUEVO: refreshMe (para luego de cambiar password)
  const refreshMe = useCallback(async () => {
    ...
    try {
      ...
      toast.error(e?.response?.data?.detail || "No se pudo refrescar tu sesión");
      return null;
    }
  }, [access]);
      ...
      try {
        ...
        try {
          ...
          if (u) {
            ...
            profile =
              (Array.isArray(data) ? data.find((it) => it.username === u) : null) ??
              { username: u, roles: [] };
          }
        }
  ...
  const login = async (username, password) => {
    ...
    try {
      ...
      try {
        ...
        profile =
          (Array.isArray(list) ? list.find((it) => it.username === u) : null) ??
          { username: u, roles: [] };
      }
